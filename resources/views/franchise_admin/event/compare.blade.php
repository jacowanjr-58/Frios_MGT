    ...
    <div class="content-body default-height p-5 mt-5">
        <div class="container-fluid rounded border p-5 bg-white">
            ...
            <table class="table customer-table display mb-4 fs-14 card-table dataTable no-footer" id="dynamicTable">
                ...
                <tbody>
                    @foreach ($event->eventItems as $key => $event)
                        <tr
                            style="{{ $event->quantity < $event->item->availableQuantity() ? 'background-color: red' : 'background-color: green' }}">
                            <td>
                                {{ $key + 1 }}
                            </td>
                            <td>
                            ...
                            <td>
                                {{ $event->item->availableQuantity() }}
                            </td>
                            <td>
                                {{ $event->quantity }}
                            </td>
                            <td class="d-flex justify-content-around">
                                @if ($event->quantity < $event->item->availableQuantity())
