                                ...
                                <tr>
                                    ...
                                    <td class="d-flex align-items-center">
                                        <input type="number" name="total_cases_{{ $master->inventory_id }}"
                                            id="total_cases_{{ $master->inventory_id }}" value="{{ $master->cases }}" min="0"
                                            class="form-control case-input" data-id="{{ $master->inventory_id }}"
                                            style="width: 12ch">
                                        <div class="small"> ({{ $master->split_factor }} units/case)</div>
                                    </td>
                                    <td>
                                        <input type="number" name="total_units_{{ $master->inventory_id }}"
                                            id="total_units_{{ $master->inventory_id }}" value="{{ $master->units }}" min="0"
                                            class="form-control unit-input" data-id="{{ $master->inventory_id }}"
                                            style="width: 12ch">
                                    </td>
                                    <td>
                                        <input type="number" name="total_quantity_{{ $master->inventory_id }}"
                                            id="total_quantity_{{ $master->inventory_id }}"
                                            value="{{ $master->total_quantity }}" class="form-control" style="width: 12ch"
                                            readonly>
                                        <input type="hidden" name="split_factor_{{ $master->inventory_id }}"
                                            id="split_factor_{{ $master->inventory_id }}" value="{{ $master->split_factor }}">
                                    </td>
                                    <td>
                                        <textarea name="notes_{{ $master->inventory_id }}"
                                            class="form-control @error('notes_'.$master->inventory_id) is-invalid @enderror"
                                            rows="2"
                                            placeholder="Enter adjustment notes here">{{ old('notes_'.$master->inventory_id, $master->notes) }}</textarea>
                                        @error('notes_'.$master->inventory_id)
