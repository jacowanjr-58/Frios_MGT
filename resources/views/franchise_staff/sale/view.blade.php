...
<div class="content-body default-height p-5 mt-5">
    <div class="container-fluid rounded border p-5 bg-white">
        ...
        <div class="d-flex justify-content-between">
            <div class="container">
                <h1 style="font-size: 28px; font-weight: bold">View Sale</h1>
                            ...
                            @foreach ($invoice->items as $index => $item)
                                <tr>
                                    <td class="text-center">
                                        <input disabled type="checkbox" name="items[{{ $index }}][taxable]" class="taxable-checkbox"
                                            {{ $item->taxable ? 'checked' : '' }}>
                                    </td>
                                    <td>{{ $item->flavor->name ?? 'N/A' }}</td>
                                    <td>{{ $item->inventory_allocation_id }}</td>
                                    <td>
                                        <input disabled type="hidden" name="items[{{ $index }}][flavor_id]" value="{{ $item->flavor_id }}">
                                        <input disabled type="hidden" name="items[{{ $index }}][location]" value="{{ $item->inventory_allocation_id }}">
                                        <input disabled type="number" name="items[{{ $index }}][quantity]" value="{{ $item->quantity }}" class="form-control form-control-sm" readonly>
                                    </td>
                                    <td>
                                        <input disabled type="hidden" name="items[{{ $index }}][price]" value="{{ $item->unit_price }}">
                                        {{ $item->unit_price }}
                                    </td>
                    ...
                    <div class="row mb-4">
                        ...
                        <div class="col-md-4 flex">
                            Tax Rate (%):
                            <input disabled type="number" class="form-control w-75" id="taxRate"
                                value="{{ $invoice->tax_price ?? $franchisee }}" min="0" max="100"
                                step="0.01" name="tax_price">
                        </div>
                        <div class="col-md-12">
                            <label for="Note">Note to customer</label>
                            <textarea disabled name="note" id="note" class="form-control">{{ old('note', $invoice->note) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="row mb-4" style="display: none">
                        ...
                        <div class="col-md-2">
                            <input disabled type="number" id="item-quantity" class="form-control" placeholder="Qty"
                                min="1" value="1">
                        </div>
                        <div class="col-md-3">
...
<script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.getElementById('add-item').addEventListener('click', () => {
                ...
                row.innerHTML = `
                <td class="text-center">
                    <input disabled type="checkbox" name="items[${index}][taxable]" class="taxable-checkbox" checked>
                </td>
                <td>${flavorName}</td>
                <td>${locationName}</td>
                <td>
                    <input disabled type="hidden" name="items[${index}][flavor_id]" value="${flavorId}">
                    <input disabled type="hidden" name="items[${index}][location]" value="${locationName}">
                    <input disabled type="number" name="items[${index}][quantity]" value="${qty}" class="form-control form-control-sm" readonly>
                </td>
                <td>
                    <input disabled type="hidden" name="items[${index}][price]" value="${price}">
                    ${price.toFixed(2)}
                </td>
